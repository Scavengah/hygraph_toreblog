...
import { isText, isElement } from '@graphcms/rich-text-types';
...
function RenderText(_ref) {
  ...
  if (bold && Bold) {
    element = React.createElement(Bold, null, element);
  }

  if (italic && Italic) {
    element = React.createElement(Italic, null, element);
  }

  if (underline && Underline) {
    element = React.createElement(Underline, null, element);
  }
...
function RenderElement(_ref2) {
  ...
  if (NodeRenderer) {
    return React.createElement(NodeRenderer, Object.assign({}, rest), React.createElement(RichText, {
      content: children,
      renderers: renderers
    }));
  }
